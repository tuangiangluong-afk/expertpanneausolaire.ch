...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    Object.values(CITIES).forEach(site => {
        if (site.slug !== 'home' && site.slug !== 'expertpanneausolaire.ch' && site.slug !== 'www.expertpanneausolaire.ch') {
    ...
    // 5. Domination Longue Traîne: City x Marque (pSEO Matrix)
    const cityMarqueRoutes: MetadataRoute.Sitemap = Array.from(uniqueSites.values()).flatMap((site) => {
        ...
        return SOLAR_BRANDS.map((marque) => ({
    ...
    // 6. Marques Pages
    const marquesRoutes: MetadataRoute.Sitemap = SOLAR_BRANDS.map((marque) => ({
        url: `${BASE_URL}/marques/${marque.slug}`,
        lastModified: new Date(),
        changeFrequency: 'weekly' as const,
        priority: 0.8,
    }));
    ...
    const typeRoutes: MetadataRoute.Sitemap = SOLAR_TYPES.map((type) => ({
    ...
    // 8. Puissances d'installation
    const puissanceRoutes: MetadataRoute.Sitemap = SOLAR_PUISSANCES.map((p) => ({
        url: `${BASE_URL}/puissance/${p.slug}`,
        lastModified: new Date(),
        changeFrequency: 'monthly' as const,
        priority: 0.8,
    }));
    ...
    // 9. Comparatifs
    const comparatifRoutes: MetadataRoute.Sitemap = SOLAR_COMPARATIFS.map((comp) => ({
