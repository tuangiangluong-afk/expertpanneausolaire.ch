...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
    ...
    if (!post) {
        return { title: 'Article non trouvé | Expert Panneau Solaire' };
    }

    return {
        title: clampTitle(post.seo_title || `${post.title} | Expert Panneau Solaire`),
        ...
        openGraph: {
            ...
            authors: [post.author_name || 'Expert Panneau Solaire'],
        },
    };
}
...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const schema = {
        ...
        "author": {
            "@type": "Person",
            "name": post.author_name || "Expert Panneau Solaire",
            "url": post.author_slug ? `https://www.expertpanneausolaire.ch/author/${post.author_slug}` : undefined,
            "@id": post.author_slug ? `https://www.expertpanneausolaire.ch/author/${post.author_slug}#person` : undefined
        },
        "publisher": {
            ...
            "logo": {
                ...
                "url": "https://www.expertpanneausolaire.ch/logo.png"
        ...
        "mainEntityOfPage": {
            ...
            "@id": `https://www.expertpanneausolaire.ch/blog/${slug}`
    ...
    const howToSchema = (headers && headers.length >= 3) ? {
        ...
        "step": headers.map((h: any, i: number) => ({
            ...
            "name": h.text,
            "text": h.text,
            "url": `https://www.expertpanneausolaire.ch/blog/${slug}#${h.id}`
        }))
    } : null;
                    ...
                    <aside className="lg:col-span-4 space-y-8">
                        ...
                        <div className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm">
                            ...
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-amber-100 flex items-center justify-center cursor-pointer hover:opacity-85 text-amber-600 font-bold overflow-hidden relative">
                                ...
                                <div>
                                    <Link href={post.author_slug ? `/author/${post.author_slug}` : "#"} className="font-bold text-neutral-900 hover:text-amber-600 transition-colors">{post.author_name || 'Expert Panneau Solaire'}</Link>
                                    <p className="text-xs text-neutral-500">Expert Solaire</p>
                                </div>
                            </div>
                        </div>
                         ...
                         <div className="bg-gradient-to-br from-blue-900 to-blue-700 text-white rounded-2xl p-8 text-center shadow-lg">
                            ...
                            <p className="text-blue-100 text-sm mb-6">Comparez les prix des installateurs du label « Les Pros du Solaire » près de chez vous.</p>
                            <Link href="/#simulateur" className="inline-block bg-white text-amber-900 px-6 py-3 rounded-full font-bold hover:bg-amber-50 transition-colors w-full">
                                Commencer
                            </Link>
                         </div>
                    </aside>
