...
interface SolaireContentPageProps {
    site: CityConfig;
    heroBadge: string;
    pageTitle: string;
    introHtml: string;
...
export default function SolaireContentPage({
    site,
    heroBadge,
    pageTitle,
    introHtml,
    ...
    const breadcrumbSchema = {
        ...
        "@type": "BreadcrumbList",
        itemListElement: [
            { "@type": "ListItem", position: 1, name: "Accueil", item: "https://www.expertpanneausolaire.ch" },
            ...breadcrumb.map((b, i) => ({
                "@type": "ListItem",
                position: i + 2,
                name: b.name,
        ...
        <div className="min-h-screen font-sans text-slate-900 bg-white">
                ...
                <div className="container mx-auto px-4">
                    <div className="grid lg:grid-cols-2 gap-12 items-center">
                        <div>
                            ...
                            <div
                                className="text-lg text-slate-600 mb-8 leading-relaxed prose prose-lg prose-amber"
                                dangerouslySetInnerHTML={{ __html: introHtml }}
                            />

                            {yieldHtml && (
                                <div className="mb-8" dangerouslySetInnerHTML={{ __html: yieldHtml }} />
                            )}

                            <div className="grid grid-cols-2 gap-4 mb-8">
                                {facts.map((f) => (
                                    <div key={f.label} className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                                        <div className="text-xs font-bold uppercase tracking-wide text-slate-500 mb-1">{f.label}</div>
                                        <div className="text-lg font-extrabold text-slate-900">{f.value}</div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-4">
                                <a
                                    href="#simulateur"
                                    className="bg-amber-500 text-white px-8 py-4 rounded-xl font-bold text-center hover:bg-amber-600 transition"
                                >
                        ...
                        <div className="relative h-[400px] lg:h-[500px] rounded-3xl overflow-hidden shadow-2xl">
                            <Image src={heroImage} alt={pageTitle} fill className="object-cover" />
                        </div>
                    </div>
                </div>
            </section>
            ...
            {benefits.length > 0 && (
                <section className="py-16 bg-slate-50 border-y border-slate-200">
                    <div className="container mx-auto px-4">
                        <h2 className="text-3xl font-bold text-slate-900 mb-10 text-center">Pourquoi nous choisir</h2>
                        <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                            {benefits.map((b) => (
                                <div key={b} className="flex items-start gap-3 bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                                    <CheckCircle size={20} className="text-amber-600 shrink-0 mt-0.5" />
                                    <p className="text-slate-700">{b}</p>
                                </div>
                            ))}
                        </div>
                        <div className="max-w-4xl mx-auto mt-8 bg-amber-50 border border-amber-100 rounded-2xl p-5 flex gap-3">
                            <Award className="text-amber-600 shrink-0 mt-1" />
                            <p className="text-sm text-slate-700 italic">
                                <strong>Conseil Expert :</strong> {expertTip}
                            </p>
                        </div>
                    </div>
                </section>
            ...
            {sections.map((s) => (
                <section key={s.title} className="py-14">
            ...
            <section className="py-16 bg-slate-50 border-y border-slate-200">
                <div className="container mx-auto px-4 max-w-4xl">
                    ...
                    <div className="space-y-4">
                        {faqs.map((faq) => (
                            <details key={faq.question} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 group">
                                ...
                                <p className="mt-4 text-slate-600 leading-relaxed">{faq.reponse}</p>
                            </details>
                        ))}
                    </div>
                </div>
            ...
            <section className="py-16" id="simulateur">
                <div className="container mx-auto px-4">
